...
```{r}
df_tr <- read_rds(paste0(data_dir, "train_clustering_rm_3592.rds"))
...
```{r}
df_tr %>% dplyr::select(label, contains("CLAIM")) -> vdf_tr
df_ts %>% dplyr::select(label, contains("CLAIM")) -> vdf_ts
...
```{r}
hc <- read_rds(paste0(results_dir, "hc_cut.rds"))
```
...
write.csv(pr_grpl_bin,paste0(results_dir,"/01_data/cluster_dfs/gilead/pr_glasso_cluster.csv"))
```
