...
results_dir <- "F:/Projects/Strongbridge/results/feature_selection/Clustering/Remove_3592/"
...
```{r}
df <- readRDS(paste0(path, "train_clustering_rm_3592.rds"))
...
# Reduce dataframe to only those variables to be used for clustering. 
# Then transpose.
vdf <- df %>% select(contains("CLAIM"))
...
```{r}
write_rds(hc, paste0(results_dir, "hc_obj.rds"))
write_rds(hc_cut, paste0(results_dir, "hc_cut.rds"))
write_rds(agg_vdf, paste0(results_dir, "x_clustering_tr_agg_sum.rds"))
write_csv(clust_memb, paste0(results_dir, "clust_memb_table.csv"), na="")
```
