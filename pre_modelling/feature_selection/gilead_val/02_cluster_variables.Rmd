...
Read the training data that has been preprocessed using ./01_preprocess.Rmd. Note that the readr package is faster but sometimes leads to errors so I've used Base R instead.  
```{r}
df <- read.csv(paste0(path, "/01_data/cluster_dfs/gilead/", "df_tr_clust.csv"), stringsAsFactors = FALSE)
...
Note - this block can be slow to run. 

```{r}
wss_all <- NULL
upper <- log2(ncol(vdf)-1)
c_seq <-  round(2^seq(1, upper, 1))
...
The number of clusters has been selected as 70. **EDIT - please manually insert the final number of clusters before proceeding.**
...
write_rds(agg_vdf, paste0(path, "/01_data/cluster_dfs/gilead/", "x_clustering_tr_agg_mean.rds"))
