...
# Read in the data
#-------------------------------------------------------------------------------
path = "F:/Projects/Strongbridge/results/modelling/XGBOOST_preliminary/"

df <- readRDS(paste0(path,"XGB_prelim_test_predictions_1_1000.rds"))

vars <- readRDS("F:/Projects/Strongbridge/data/modelling/preliminary_model_data/01_prelim_matched_combined_train_test.rds")
...
rel_risks[[1]] <- relative_risk(rr_col = "AGE", bin_size_o = 10, c_min_o = 0)
...
rel_risks[[5]] <- relative_risk(rr_col = "D_V053_AVG_CLAIM_CNT", bin_size_o = 0.5, c_min_o = 0, include_lowest = F)
rel_risks[[6]] <- relative_risk(rr_col = "P_99214_AVG_CLAIM_CNT", bin_size_o = 1, c_min_o = 0, include_lowest = F)
rel_risks[[7]] <- relative_risk(rr_col = "G_371000_AVG_CLAIM_CNT", bin_size_o = 1, c_min_o = 0, include_lowest = F)
rel_risks[[8]] <- relative_risk(rr_col = "D_7288_AVG_CLAIM_CNT", bin_size_o = 1, c_min_o = 0, include_lowest = F)
rel_risks[[9]] <- relative_risk(rr_col = "D_7807_AVG_CLAIM_CNT", bin_size_o = 1, c_min_o = 0, include_lowest = F)
rel_risks[[10]] <- relative_risk(rr_col = "G_372000_AVG_CLAIM_CNT", bin_size_o = 1, c_min_o = 0, include_lowest = F)
rel_risks[[11]] <- relative_risk(rr_col = "D_3449_AVG_CLAIM_CNT", bin_size_o = 1, c_min_o = 0, include_lowest = F)
rel_risks[[12]] <- relative_risk(rr_col = "S_S21_AVG_CLAIM", bin_size_o = 1, c_min_o = 0, include_lowest = F)
rel_risks[[13]] <- relative_risk(rr_col = "P_99213_AVG_CLAIM_CNT", bin_size_o = 1, c_min_o = 0, include_lowest = F)
rel_risks[[14]] <- relative_risk(rr_col = "P_99285_AVG_CLAIM_CNT", bin_size_o = 1, c_min_o = 0, include_lowest = F)
rel_risks[[15]] <- relative_risk(rr_col = "S_S44_AVG_CLAIM", bin_size_o = 1, c_min_o = 0, include_lowest = F)
rel_risks[[16]] <- relative_risk(rr_col = "G_394000_AVG_CLAIM_CNT", bin_size_o = 1, c_min_o = 0, include_lowest = F)
rel_risks[[17]] <- relative_risk(rr_col = "S_S37_AVG_CLAIM", bin_size_o = 1, c_min_o = 0, include_lowest = F)
...
write.csv(rel_risks_all, paste0(path, "XGB_prelim_relative_risks.csv") , 
          row.names = F)
