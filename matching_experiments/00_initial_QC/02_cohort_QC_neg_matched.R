...
base_dir_2 <- 'F:/Projects/Strongbridge/data/matching_experiments/pre_modelling/'
...
df_all <- df_all[df_all$subset == 'train_neg',]%>% 
          dplyr::mutate(PATIENT_ID=as.numeric(PATIENT_ID)) %>% 
          dplyr::arrange(PATIENT_ID)
...
# CHECK 3 -- ---------------------------------------------------
  if (any(first_lt_12)){
    ...
    dd_passed <- all(df_12_dd_sel_patient[first_dd] == df_all_sel_patient_dd[first] & df_12_dd_sel_patient[last_dd] == df_all_sel_patient_dd[last])
    #Expect larger frequency for df_12, 0 for df_13
